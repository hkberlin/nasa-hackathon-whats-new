...
def load_data(year: int, folder: str):
	...
	x = np.load(os.path.join(folder, f"filled-{year}-mfi-BGSE.npy"), allow_pickle=True)
	x_epoch = np.load(os.path.join(folder, f"{year}-mfi-Epoch.npy"), allow_pickle=True)
	x_epoch = [e[0] for e in x_epoch]
	# x_prime (mag) [86400 * N_days, 3]
	x_prime = np.load(os.path.join(folder, f"filled-{year}-mag-B1GSE.npy"), allow_pickle=True)

	# y (swe) [570, 3]
	n = np.expand_dims(np.load(os.path.join(folder, f"filled-{year}-swe-Proton_Np_moment.npy"), allow_pickle=True), axis=1)
	v = np.expand_dims(np.load(os.path.join(folder, f"filled-{year}-swe-Proton_V_moment.npy"), allow_pickle=True), axis=1)
	w = np.expand_dims(np.load(os.path.join(folder, f"filled-{year}-swe-Proton_W_moment.npy"), allow_pickle=True), axis=1)
...
def interpolate(epoch, data, N):
	...
	return interp_f(u_epoch), u_epoch
...
def cut(x: np.array, x_prime: np.array, y: np.array, y_epoch: np.array,
		seq_len: int,
		threshold: float,
	...
	dataset = []
	for i in range(0, total_length - seq_len, stride):
		...
		if True:
			...
			y_cut_epoch = y_epoch[i:i+seq_len]
			dataset.append((x_sequence, x_prime_sequence, y_sequence, y_cut_epoch))
...
def save(inputData: List[Tuple[np.array, np.array, np.array]], save_path: str, year: int) :
	...
	if len(inputData) == 1:
		...
		outputDataY = inputData[0][2][:]
		outputDataY_epoch = inputData[0][3]
	else:
		outputDataxx = np.stack( (inputData[0][0],inputData[1][0]), axis = 0)
		outputDataX =  np.stack( (inputData[0][1],inputData[1][1]), axis = 0)
		outputDataY =  np.stack( (inputData[0][2][:],inputData[1][2][:]), axis = 0)
		print("print save info")
		print(inputData[0][3])
		print(inputData[0][3].shape)
		print(inputData[1][3])
		print(inputData[1][3].shape)
		outputDataY_epoch = np.stack((inputData[0][3], inputData[1][3]), axis = 0)
		...
		for i in range(len(inputData)-2):
			...
			outputDataY =  np.concatenate( (outputDataY,inputData[i+2][2].reshape(1,n_seq,3) ), axis = 0)
			print(outputDataY_epoch.shape)
			print(inputData[i+2][3].shape)
			outputDataY_epoch = np.concatenate((outputDataY_epoch, inputData[i+2][3].reshape(1, n_seq)), axis = 0)
	...
	outputDataY_epoch = torch.tensor(outputDataY_epoch)
	dic = dict({
		'xx':outputDataxx,
		'X': outputDataX,
		'Y': outputDataY,
		'date': outputDataY_epoch
	})
...
def parse_args() -> Namespace:
	...
	# load_data
	parser.add_argument("--data_path", type=str, default="npFiles")
	...
	# cut
	parser.add_argument("--seq_len", type=int, default=24*30)
	parser.add_argument("--similarity_threshold", type=float, default=0.5)

	# paths
	parser.add_argument("--save_path", type=str, default="Data/processed-date")
	parser.add_argument("--plot_path", type=str, default="Data/plot-date")
...
def main(args):
	for year in range(2016, 2016+1):
		...
		x, x_inp_epoch = interpolate(x_epoch, x, x.shape[0])
		y, y_inp_epoch = interpolate(y_epoch, y, y.shape[0])
		...
		y_inp_epoch = downsample(y_inp_epoch, num_of_day, args.frequency)
		print("after downsample, x:", x.shape)
		print("after downsample, x_prime:", x_prime.shape)
		print("after downsample, y:", y.shape)
		print("after downsample, y_epoch:", y_inp_epoch.shape)

		# cut
		dataset = cut(x, x_prime, y, y_inp_epoch, args.seq_len, args.similarity_threshold, args.similarity_metric)
